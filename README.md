...
- This project depends on [Ultralytics](https://github.com/ultralytics/ultralytics) YOLOv8 for its primary person/ball detector, its pose estimator, and its pitch-keypoint calibration model (`src/perception/yolo_detector.py`, `pose_estimator.py`, `pitch_keypoint_calibration.py`) -- Ultralytics is distributed under **AGPL-3.0**. Disclosed here as a factual statement of the project's current dependency posture, independent of any other component's licensing (e.g. the `roboflow/sports` tooling used elsewhere is MIT).
